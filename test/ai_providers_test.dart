  ...
  group('LocalAIProvider', () {
    test('generate returns content on success', () async {
      ...
      final verification = verify(mockClient.post(
        argThat(predicate((Uri uri) => uri.toString() == 'http://localhost:11434/v1/chat/completions')),
    ...
    test('generate throws exception on error', () async {
      ...
      expect(
        () => localProvider.generate([const ChatMessage(role: MessageRole.user, content: 'Hi')]),
        throwsException,
      );
    });
  });
