...
final aiProviderServiceProvider = FutureProvider<AIProvider?>((ref) async {
  ...
  switch (selectedProvider) {
    case 'openai':
      ...
      if (key != null && key.isNotEmpty) return OpenAIProvider(apiKey: key, model: model);
      break;
    case 'anthropic':
      final key = await keystore.retrieve('ANTHROPIC_API_KEY');
      final model = await keystore.retrieve('ANTHROPIC_MODEL') ?? 'claude-3-5-haiku-20241022';
      if (key != null && key.isNotEmpty) return AnthropicProvider(apiKey: key, model: model);
      break;
    case 'groq':
      final key = await keystore.retrieve('GROQ_API_KEY');
      final model = await keystore.retrieve('GROQ_MODEL') ?? 'llama3-70b-8192';
      if (key != null && key.isNotEmpty) return GroqProvider(apiKey: key, model: model);
      break;
    case 'openrouter':
      final key = await keystore.retrieve('OPENROUTER_API_KEY');
      final model = await keystore.retrieve('OPENROUTER_MODEL') ?? 'openai/gpt-4o-mini';
      if (key != null && key.isNotEmpty) return OpenRouterProvider(apiKey: key, model: model);
      break;
    case 'local':
    ...
    default:
      ...
      final modelName = await keystore.retrieve('GEMINI_MODEL') ?? 'gemini-2.5-flash';
      if (key != null && key.isNotEmpty) return GoogleGeminiProvider(apiKey: key, modelName: modelName);
      break;
  }
