...
class EditorScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(filePath.split('/').last, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppThemes.textPrimary)),
        backgroundColor: AppThemes.surfaceDark,
        elevation: 0,
        iconTheme: const IconThemeData(color: AppThemes.textPrimary),
        actions: [
          IconButton(
            icon: const Icon(Icons.auto_awesome, color: AppThemes.accentBlue),
          ...
          IconButton(icon: const Icon(Icons.save, color: AppThemes.textSecondary), onPressed: () {}),
        ],
      ),
  ...
  void _showAiPromptDialog(BuildContext context, WidgetRef ref) {
    ...
    showDialog(
      ...
      builder: (ctx) {
        return AlertDialog(
          backgroundColor: AppThemes.surfaceDark,
          title: const Text('AI Quick Mod', style: TextStyle(color: AppThemes.textPrimary)),
          content: TextField(
             ...
             decoration: InputDecoration(
               ...
               enabledBorder: const OutlineInputBorder(borderSide: BorderSide(color: AppThemes.dividerColor)),
               focusedBorder: const OutlineInputBorder(borderSide: BorderSide(color: AppThemes.accentBlue)),
             ),
             maxLines: 3,
          ),
          actions: [
            TextButton(
              ...
              child: const Text('Cancel', style: TextStyle(color: AppThemes.textSecondary)),
            ),
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: AppThemes.accentBlue),
              onPressed: () {
