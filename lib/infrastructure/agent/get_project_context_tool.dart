...
class GetProjectContextTool implements AgentTool {
  ...
  Future<String> execute(Map<String, dynamic> params) async {
    ...
    try {
      ...
      final projectDir = Directory(p.join(appDir.path, 'Projects', projectId));
      ...
      void scanDir(Directory currentDir, String prefix) {
        final entities = currentDir.listSync()..sort((a, b) => p.basename(a.path).compareTo(p.basename(b.path)));
        for (var i = 0; i < entities.length; i++) {
          ...
          if (name == 'build' || name == '.dart_tool' || name == '.git' || name == 'linux' || name == 'windows' || name == 'macos' || name == 'web') {
             continue; 
          }
